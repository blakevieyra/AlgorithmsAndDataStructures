...
#Generate the menu of options, replays until exit is chosen. Options call on the methods below.
  ...
  def run_statistics(self):
    ...
    max_category = expense_categories[expense_values.index(
        max_expense)] if max_expense > 0 else "None"
    min_category = expense_categories[expense_values.index(
        min_expense)] if min_expense > 0 else "None"
  ...
  def budget_analysis(self, max_expense, avg_expense, std_deviation, cv):
    ...
    if not hasattr(self, 'income') or self.income <= 0:
      analysis_message.append(
          "Income is zero or not set, unable to compare expenses to income.")
    ...
    if cv > 50:
      ...
      analysis_message.append(
          "Expenses are relatively stable. Keep tracking to maintain budget health."
      )
    ...
    if std_deviation > 0.5 * avg_expense:
      ...
      analysis_message.append(
          "Your month-to-month expenses are fairly consistent.")
    ...
    if max_expense > 2 * avg_expense:
      ...
      analysis_message.append(
          "Your highest expense is within a normal range compared to your average expenses."
      )
    ...
    for category, amount in self.bill_account.get_bills().items():
      ...
      if percentage_of_total > 15:
        analysis_message.append(
            f"Consider reducing {category}, which accounts for {percentage_of_total:.2f}% of your total expenses."
        )
      if percentage_of_income > 10:
        analysis_message.append(
            f"Alert: {category} expenses take up more than 10% of your income. Consider budget adjustments."
        )
